...
class ReplayBuffer(Dataset):
    ...
    @classmethod
    def create(cls, obs_space, act_dim, size):
        """Create a empty replay buffer.
        ...
        # buffer_dict = {
        #     "observations": np.zeros((size, *obs_space.shape), dtype=obs_space.dtype),
        #     "actions": np.zeros((size, act_dim), dtype=np.float32),
        #     "next_observations": np.zeros((size, *obs_space.shape), dtype=obs_space.dtype),
        ...
        transition = {
            "observations": np.zeros(obs_space.shape, dtype=obs_space.dtype),
            "actions": np.zeros((act_dim,), dtype=np.float32),
            "next_observations": np.zeros(obs_space.shape, dtype=obs_space.dtype),
            "terminals": np.zeros((), dtype=np.float32),
            "rewards": np.zeros((), dtype=np.float32),
            "masks": np.zeros((), dtype=np.float32),
        }

        return cls.create_from_transition(transition, size)

    @classmethod
    def create_from_transition(cls, transition, size):

...
from utils.networks import ActorVectorField, Value, Actor, ValueGrad
...
class SVGD_VGF:
    def __init__(self, q_grad, alpha, optimizer: optax.GradientTransformation):
        """
    ...
    def phi(self, obs, particles):
        ...
        obs_flatten = obs.reshape(-1, obs.shape[-1])                        # [B*N, D]
        action_flatten = particles.reshape(-1, particles.shape[-1])               # [B*N, D]
        score = self.q_grad(obs_flatten, action_flatten)
        phi_val = score.reshape(particles.shape[0], particles.shape[1], -1)
...
class AVGFAgent(flax.struct.PyTreeNode):
    """Amortized Value Gradient Flow (AVGF) agent."""
    ...
    def update(self, batch):
        ...
        def critic_loss_fn(critic_params):
            ...
            # value gradient flow
            svgd = SVGD_VGF(self.q_grad, self.config['vgf_alpha'], optax.chain(optax.clip_by_global_norm(1.0), optax.adam(learning_rate=self.config['vgf_lr'])))
        ...
        def q_grad_loss_fn(q_grad_params):
            """q_grad loss."""
            batch_size, action_dim = batch['actions'].shape

            def sum_q(actions):
                qs = self.critic(batch['observations'], actions)      
                q = jnp.mean(qs, axis=0)               
                return jnp.sum(q)                     

            score = jax.grad(sum_q)(batch['actions'])                         # [B, D]
            if self.config['q_grad_norm']:
                score = score / (jnp.linalg.norm(score, axis=-1, keepdims=True) + 1e-8)
            score = jax.lax.stop_gradient(score)

            q_grad = self.q_grad(batch['observations'], batch['actions'], params=q_grad_params)
            q_grad_loss = ((q_grad - score) ** 2).mean()

            return q_grad_loss, {
                'q_grad': q_grad.mean(),
                'score': score.mean(),
                'q_grad_loss': q_grad_loss,
            }
        ...
        new_q_grad, q_grad_info = self.q_grad.apply_loss_fn(loss_fn=q_grad_loss_fn)

        return self.replace(rng=new_rng, critic=new_critic, target_critic=new_target_critic, actor_bc=new_actor_bc, q_grad=new_q_grad), {
            **critic_info, **actor_bc_info, **q_grad_info
        }
    ...
    def create(
        ...
        rng, actor_key, critic_key, q_grad_key = jax.random.split(rng, 4)
        ...
        if config['encoder'] is not None:
            ...
            encoders['q_grad'] = encoder_module()
        ...
        actor_bc = TrainState.create(actor_bc_def, actor_bc_params, tx=optax.chain(optax.clip_by_global_norm(1.0), optax.adam(learning_rate=config['actor_lr'])))
        ...
        critic = TrainState.create(critic_def, critic_params, tx=optax.chain(optax.clip_by_global_norm(1.0), optax.adam(learning_rate=config['value_lr'])))
        target_critic = TrainState.create(critic_def, critic_params)

        q_grad_def = ValueGrad(
            hidden_dims=config['value_hidden_dims'],
            action_dim=action_dim,
            layer_norm=config['layer_norm'],
            activations=config['activations'],
            encoder=encoders.get('q_grad'),
        )
        q_grad_params = q_grad_def.init(q_grad_key, ex_observations, ex_actions)['params']
        q_grad = TrainState.create(q_grad_def, q_grad_params, tx=optax.chain(optax.clip_by_global_norm(1.0), optax.adam(learning_rate=config['value_lr'])))

        config['ob_dims'] = ob_dims
        config['action_dim'] = action_dim
        return cls(rng, critic=critic, target_critic=target_critic, actor_bc=actor_bc, q_grad=q_grad, config=flax.core.FrozenDict(**config))


def get_config():
    config = ml_collections.ConfigDict(
        dict(
            agent_name='avgf',  # Agent name.
